...
export default function Recorder({ currentWord, currentIndex, totalWords, sessionStarted }: RecorderProps) {
  ...
  const setupSpeechRecognition = () => {
    ...
    recognition.onspeechstart = () => {
      console.log("Speech detected - ready to transcribe");
  ...
  const pauseRecording = () => {
    if (recognitionRef.current && recording) {
      ...
      setPaused(true);
      setRecording(false);
      // Tidak menghentikan stream atau audio context agar bisa resume
    }
  };

  const resumeRecording = async () => {
    if (paused && recorderRef.current && streamRef.current) {
      setPaused(false);
      setRecording(true);
  ...
  return (
    <div className="w-full flex justify-center items-center">
      <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-8 w-full max-w-lg flex flex-col items-center border border-indigo-100 relative">
        ...
        {!sessionStarted && (
          <div className="w-32 h-32 rounded-full flex flex-col items-center justify-center text-white text-sm font-medium bg-gradient-to-br from-indigo-500 to-blue-500 border-4 border-indigo-300 mb-6 shadow-lg">
            <span className="text-7xl mb-0">🎙️</span>
            <span className="text-center text-xs px-2 text-white font-semibold">Klik "Mulai" untuk merekam</span>
          </div>
        )}
